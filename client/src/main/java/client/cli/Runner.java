...
public class Runner {
    ...
    public ExecutionResponse launchCommand(String[] userCommand) {
        ...
        if (userCommand[0].equals("save")) {
            return new ExecutionResponse(false, "Команда save доступна только серверу");
        }
        CommandName command;
        try {
        ...
        switch (userCommand[0]) {
            case "execute_script":
                ExecutionResponse response1 = new ExecuteScriptCommand().execute(userCommand);
            ...
            case "exit":
                return new ExitCommand().execute(userCommand);
            case "help":
                return new HelpCommand(commands).execute(userCommand);
            default:
